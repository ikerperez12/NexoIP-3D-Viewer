...
| Medium | Bounded catalog preflight is deliberately not a full parser | `electron/model-preflight.js`, `src/components/FileLibrarySidebar.jsx`, `scripts/packaged-fixture-matrix.mjs` | Prefix-and-suffix structure checks reject common malformed large glTF candidates, but dependency failures or unusual metadata can still be listed as prechecked until the loader validates them on open | Keep the UI wording explicit and retain full parser/resource validation on open; the packaged missing-MTL scenario must continue proving safe rejection and recovery |
...
| --- | --- | --- | --- | --- |
| Demand-render scheduler | `src/utils/render-loop.js`, `src/components/Viewport3D.jsx`, `electron/packaged-self-test.js` | Idle frames stop when no animation, interaction, damping or auto-rotation is active; a controlled packaged loss probe replaces the WebGL generation and reloads the model | Regressions can reintroduce background work or leaks across longer sessions | Preserve scheduler and packaged recovery tests; add a measured frame/energy and long-switch baseline before stable |
...
| Authoritative session catalog held in memory | `electron/file-scanner.js`, `src/App.jsx`, `src/components/FileLibrarySidebar.jsx` | The renderer receives bounded revisioned catalog/tree pages and expands folders lazily. The first safe discovery appears immediately, then dense updates coalesce to at most one notification every 200 ms; no full-library IPC refresh remains | The private main-process registry and sort/index work still grow with catalog size | Benchmark very large real libraries; evolve the internal index only if measured memory or query latency warrants it |
