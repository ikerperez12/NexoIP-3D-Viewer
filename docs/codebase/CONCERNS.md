...
| Medium | Bounded catalog preflight is not a full parser | `electron/model-preflight.js`, `src/components/FileLibrarySidebar.jsx` | Oversized metadata or dependency failures can still be listed as prechecked until the loader rejects them on open | Keep the UI wording explicit, retain full parser/resource validation on open, and evaluate streaming validators only with hostile-corpus benchmarks |
