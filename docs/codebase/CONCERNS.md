...
| Medium | Bounded catalog preflight is deliberately not a full parser | `electron/model-preflight.js`, `src/components/FileLibrarySidebar.jsx`, `scripts/packaged-fixture-matrix.mjs` | A dependency failure or unusual metadata can still be listed as prechecked until the loader validates it on open | Keep the UI wording explicit and retain full parser/resource validation on open; the packaged missing-MTL scenario must continue proving safe rejection and recovery |
