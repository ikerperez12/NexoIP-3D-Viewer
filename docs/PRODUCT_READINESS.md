...
Current evidence: on the development Windows host, the unpacked executable plus freshly built NSIS and portable artifacts all load the same ten representative scenarios across every advertised extension, including Draco, Meshopt and KTX2/Basis, through the local renderer and opaque-ID model protocol. All three forms also reject the six compact hostile matrix candidates before catalog publication. Each then publishes a structurally valid OBJ with a missing MTL dependency, verifies the complete primary asset through the private protocol, observes a path-free loader error without a false loaded marker, and recovers to a usable library and canvas. A controlled synthetic WebGL-loss probe additionally verifies the visible recovery action, replacement renderer generation and canvas, model reload and healthy replacement context; it does not substitute for constrained-GPU or endurance evidence. The guarded NSIS smoke installs, self-tests and uninstalls in a unique temporary directory; the portable smoke uses the same capability contract. Both leave no NexoIP process or smoke root behind. The packaged self-test also exercises targeted keyboard, semantic and no-global-overflow invariants at 900x600 and 200% browser zoom; this is not a complete WCAG conformance audit. Clean-profile Windows 10/11, assistive-technology and constrained-GPU evidence remains open for stable.
